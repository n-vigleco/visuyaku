...
生まれ年から数え年を計算し、ひとりまたは複数人の前厄・本厄・後厄を同じ時間軸で比較できる静的Webアプリです。
...
- 生まれ年と性別から対象年の数え年を計算
- 男性・女性の厄年を同じグラフに表示
- A/B/Cの匿名ラベルで最大10人を比較
- 誰かの厄年が連続する「厄年リレー」期間を自動検出
- 前後1年を月単位で表示
- 前後12年、80歳までの長期表示
- 選択した年と現在位置をグラフ上に表示
...
| `mode` | 複数人表示では `group` | `group` |
| `people` | 生まれ年と性別（`m` / `f`）のカンマ区切り | `1984m,1986m` |
| `year` | グラフの基準年 | `2026` |
| `view` | `1y`、`12y`、`80y` | `1y` |
...
```text
https://tmp.vigle.co/visuyaku/?mode=group&people=1984m,1986m,1988m,1990m&year=2026&view=12y
```

人物ラベルは入力順にA〜Jを自動付与します。共有URLに氏名や生年月日は含まれません。
